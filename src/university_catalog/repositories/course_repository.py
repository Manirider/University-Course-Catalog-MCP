...
from university_catalog.models import Course, Department, Instructor, Prerequisite
...
class CourseRepository:
    ...
    def search_courses(
        ...
        if department_code:
            stmt = stmt.filter(
                func.lower(Department.code) == department_code.strip().lower()
            )
    ...
    def get_by_code(self, course_code: str) -> Course | None:
        return (
            self.session.query(Course)
            .filter(func.lower(Course.course_code) == course_code.strip().lower())
            .first()
        )
    ...
    def get_prerequisite_edges(self, course_code: str) -> list[tuple]:
        ...
        edges = (
            self.session.query(Prerequisite)
            .filter(
...
class DepartmentRepository:
    ...
    def get_by_code(self, code: str) -> Department | None:
        return (
            ...
            .filter(func.lower(Department.code) == code.strip().lower())
            .first()
        )
...
class InstructorRepository:
    ...
    def get_by_name(self, name: str) -> Instructor | None:
        return (
            self.session.query(Instructor)
            .filter(func.lower(Instructor.name) == name.strip().lower())
            .first()
        )
