...
async def test_mcp_tools_registered():
    ...
    expected_tools = {"search_courses", "get_prerequisites", "lookup_instructor", "get_prerequisite_graph"}
...
async def test_mcp_resources_registered():
    ...
    expected_resources = {"resource://course_descriptions", "resource://department_directory"}
...
async def test_mcp_tool_lookup_instructor():
    result = await mcp_server.call_tool("lookup_instructor", {"instructor_name": "Dr. Alice Smith"})
    assert isinstance(result, list)
    assert len(result) > 0
...
async def test_mcp_prompt_course_comparison():
    result = await mcp_server.get_prompt("course_comparison_template", {"course_code_1": "CS101", "course_code_2": "CS102"})
