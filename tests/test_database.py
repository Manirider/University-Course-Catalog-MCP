...
from university_catalog.database import get_engine, init_db
...
def test_foreign_key_integrity(setup_database):
    ...
    for prereq in prereqs:
        assert (
            session.query(Course).filter(Course.id == prereq.course_id).first()
            is not None
        )
        assert (
            session.query(Course).filter(Course.id == prereq.prerequisite_id).first()
            is not None
        )
    ...
    for inst in instructors:
        assert (
            session.query(Department)
            .filter(Department.id == inst.department_id)
            .first()
            is not None
        )
...
def test_no_self_prerequisites(setup_database):
    ...
    prereqs = (
        ...
        .filter(Prerequisite.course_id == Prerequisite.prerequisite_id)
        .all()
    )
    session.close()
